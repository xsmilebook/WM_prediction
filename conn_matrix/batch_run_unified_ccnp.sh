#!/bin/bash
#SBATCH --job-name=unified_ccnp
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=2
#SBATCH --partition=q_cn
#SBATCH --array=1-193%193            # TODO: 修改 100 为实际被试数量 (根据 sublist.txt 行数)
#SBATCH --output=/ibmgpfs/cuizaixu_lab/xuhaoshu/code/WM_prediction/log/conn_matrix/unified_ccnp/unified_ccnp_%A_%a.out
#SBATCH --error=/ibmgpfs/cuizaixu_lab/xuhaoshu/code/WM_prediction/log/conn_matrix/unified_ccnp/unified_ccnp_%A_%a.err

# ================= 配置区域 =================
PROJECT_ROOT="/ibmgpfs/cuizaixu_lab/xuhaoshu/code/WM_prediction"

# 1. 设置被试列表文件路径 (CCNP)
SUBJ_LIST="${PROJECT_ROOT}/data/CCNP/table/sublist.txt"

# 2. 设置 Python 脚本路径 (使用统一脚本)
PYTHON_SCRIPT="${PROJECT_ROOT}/src/conn_matrix/process_dataset_unified.py"

# 3. 设置数据路径 (CCNP 路径)
DATASET_PATH="${PROJECT_ROOT}/data/CCNP"
MASK_OUTPUT_DIR="${PROJECT_ROOT}/data/CCNP/mri_data/wm_postproc"
FC_OUTPUT_DIR="${PROJECT_ROOT}/data/CCNP/fc_matrix/individual"
Z_OUTPUT_DIR="${PROJECT_ROOT}/data/CCNP/fc_matrix/individual_z"

# 图谱路径 (使用本地 reslice_atlases.py 生成的文件)
ATLAS_DIR="${PROJECT_ROOT}/data/atlas/resliced_ccnp"
