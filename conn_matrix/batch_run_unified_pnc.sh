...
# 1. 设置被试列表文件路径 (PNC)
SUBJ_LIST="${PROJECT_ROOT}/data/PNC/table/sublist.txt"

# 2. 设置 Python 脚本路径 (使用统一脚本)
PYTHON_SCRIPT="${PROJECT_ROOT}/src/conn_matrix/process_dataset_unified.py"

# 3. 设置数据路径 (PNC 路径)
DATASET_PATH="${PROJECT_ROOT}/data/PNC"
MASK_OUTPUT_DIR="${PROJECT_ROOT}/data/PNC/mri_data/wm_postproc"
FC_OUTPUT_DIR="${PROJECT_ROOT}/data/PNC/fc_matrix/individual"
Z_OUTPUT_DIR="${PROJECT_ROOT}/data/PNC/fc_matrix/individual_z"

# 图谱路径 (使用本地 reslice_atlases.py 生成的文件)
ATLAS_DIR="${PROJECT_ROOT}/data/atlas/resliced_pnc"
