#!/bin/bash
#SBATCH --job-name=unified_efny
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=2
#SBATCH --partition=q_cn
#SBATCH --array=1-522%522            # TODO: 修改 100 为实际被试数量 (根据 sublist.txt 行数)
#SBATCH --output=/ibmgpfs/cuizaixu_lab/xuhaoshu/code/WM_prediction/log/conn_matrix/unified_efny/unified_efny_%A_%a.out
#SBATCH --error=/ibmgpfs/cuizaixu_lab/xuhaoshu/code/WM_prediction/log/conn_matrix/unified_efny/unified_efny_%A_%a.err

# ================= 配置区域 =================
PROJECT_ROOT="/ibmgpfs/cuizaixu_lab/xuhaoshu/code/WM_prediction"

# 1. 设置被试列表文件路径 (EFNY)
SUBJ_LIST="${PROJECT_ROOT}/data/EFNY/table/sublist.txt"
