...
为保证不同任务图之间的视觉一致性，脚本以 `age` 图作为参考，按分组数量动态换算半边小提琴图、箱线图及组内左右偏移的横向参数。也就是说，不同任务使用的不是固定数据坐标宽度，而是固定视觉宽度；同时整张图的画布宽度也会随分组数量缩放，因此只包含 `ADHD` 的 `pfactor` 图会比 `age` 图明显更窄。三张图的高度统一并较之前进一步压低。
