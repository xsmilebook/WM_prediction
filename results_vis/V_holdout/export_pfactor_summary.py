...
def parse_args():
    ...
    parser.add_argument(
        '--output_root',
        default=None,
        help='Directory for exported CSV and MAT summaries. Default: --prediction_root.',
    )
    parser.add_argument(
        '--output_prefix',
        default=None,
...
def compute_partial_series(base_folder, time_ids):
    ...
    for run_idx, run_id in enumerate(time_ids):
        ...
        if not (
            ...
            warnings.warn('Test index mismatch at Time_{}. Skipping.'.format(run_id))
            continue
...
def build_mat_cells(results_summary, all_data):
    ...
    for idx, res in enumerate(results_summary):
        target_str = res['targetStr']

        cell_r_gg[idx, 0] = target_str
        cell_r_gg[idx, 1] = all_data['R_gg'][target_str]['Corr']
        cell_r_gg[idx, 2] = all_data['R_gg'][target_str]['MAE']

        cell_r_gw[idx, 0] = target_str
        cell_r_gw[idx, 1] = all_data['R_gw'][target_str]['Corr']
        cell_r_gw[idx, 2] = all_data['R_gw'][target_str]['MAE']

        cell_r_ww[idx, 0] = target_str
        cell_r_ww[idx, 1] = all_data['R_ww'][target_str]['Corr']
        cell_r_ww[idx, 2] = all_data['R_ww'][target_str]['MAE']

        cell_partial_gw[idx, 0] = target_str
        cell_partial_gw[idx, 1] = all_data['partialR_gw'][target_str]

        cell_partial_ww[idx, 0] = target_str
        cell_partial_ww[idx, 1] = all_data['partialR_ww'][target_str]

        cell_observed_results[idx, 0] = target_str
        ...
        cell_significance_results[idx, 0] = target_str
