...
def compute_rank_indices(corr_values):
    temp = corr_values.copy()
...
def compute_partial_series(base_folder, time_ids, corr_actual_gg, corr_actual_gw, corr_actual_ww, num_folds):
    ...
    ind_gg = compute_rank_indices(corr_actual_gg)
    ind_gw = compute_rank_indices(corr_actual_gw)
    ind_ww = compute_rank_indices(corr_actual_ww)

    for rank_idx in range(len(time_ids)):
        if (
            np.isnan(corr_actual_gg[ind_gg[rank_idx]])
            or np.isnan(corr_actual_gw[ind_gw[rank_idx]])
            or np.isnan(corr_actual_ww[ind_ww[rank_idx]])
        ):
            continue

        run_id_gg = time_ids[ind_gg[rank_idx]]
        run_id_gw = time_ids[ind_gw[rank_idx]]
        run_id_ww = time_ids[ind_ww[rank_idx]]

        gg_idx, gg_pred, _ = load_all_folds(base_folder, run_id_gg, 'GGFC', num_folds)
        gw_idx, gw_pred, gw_test = load_all_folds(base_folder, run_id_gw, 'GWFC', num_folds)
        ww_idx, ww_pred, ww_test = load_all_folds(base_folder, run_id_ww, 'WWFC', num_folds)
        ...
        if not (
            ...
            warnings.warn(f'Index mismatch at rank {rank_idx}. Skipping.')
            continue
